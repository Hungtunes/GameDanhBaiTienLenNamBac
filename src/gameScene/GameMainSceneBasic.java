...
public class GameMainSceneBasic<T extends TienLenGame> {
	private T gameType;
	...
	public Scene createScene() {
		...
		if (!(gameType.getNowPlayer() instanceof AiPlayer)) {
			...
			if (tempBot.AiPlay(gameType)) {
				...
				for (Card card : gameType.getLastPlay().getCards()) {
		...
		playButton.setOnAction(e -> {
			if (gameType.isHit()) {
				...
				StringBuilder selectedCardsText = new StringBuilder();
				for (Card card : gameType.getSelectionCard().getCards()) {
					selectedCardsText.append(card.toString()).append(", ");
				}
				if (selectedCardsText.length() > 0) {
					selectedCardsText.setLength(selectedCardsText.length() - 2); // Xóa dấu ", " cuối cùng
		...
		cancelButton.setOnAction(e -> {
			if (gameType.getSelectionCard().getCards().isEmpty()) {
