	...
	public static Scene create(Stage primaryStage, int k) {
		...
		next.setOnAction(e -> {
			...
			if (numOfPlayer <= 1 || numOfPlayer > 4) {
				...
				if (k == 0) {
					TienLenGame tienLenGame = new TienLenMienNam(numOfPlayer, aiCount);
					GameMainScene<TienLenMienNam> gameScene = new GameMainScene<>((TienLenMienNam) tienLenGame);
					...
					GameMainSceneBasic<TienLenMienNam> gameScene = new GameMainSceneBasic<>(
							(TienLenMienNam) tienLenGame, primaryStage);
					Scene gameSceneObject = gameScene.createScene(); // Sử dụng GameScene để tạo gameScene
					primaryStage.setScene(gameSceneObject);
				} else if (k == 2) {
					TienLenGame tienLenGame = new TienLenMienBac(numOfPlayer, aiCount);
					GameMainScene<TienLenMienBac> gameScene = new GameMainScene<>((TienLenMienBac) tienLenGame);
					Scene gameSceneObject = gameScene.createScene(); // Sử dụng GameScene để tạo gameScene
					primaryStage.setScene(gameSceneObject);
				} else {
					TienLenGame tienLenGame = new TienLenMienBac(numOfPlayer, aiCount);
					GameMainSceneBasic<TienLenMienBac> gameScene = new GameMainSceneBasic<>(
							(TienLenMienBac) tienLenGame, primaryStage);
					Scene gameSceneObject = gameScene.createScene(); // Sử dụng GameScene để tạo gameScene
					primaryStage.setScene(gameSceneObject);
				}
			}
